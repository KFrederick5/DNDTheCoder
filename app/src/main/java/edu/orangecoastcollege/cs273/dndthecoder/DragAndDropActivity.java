...
int llchildCount = dragLayout.getChildCount();
        for(int i = 0; i<llchildCount; i++)
        {
            ...
            {
                LinearLayout childLinerLayout = (LinearLayout) childView;
                childLinerLayout.setOnDragListener(new CoderDragListener());
                int ivChildCount = childLinerLayout.getChildCount();
                for(int j =0; j< ivChildCount; j++)
                {
                    View grandchildView = childLinerLayout.getChildAt(j);
